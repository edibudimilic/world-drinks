...
| `npm run validate:data` | Validate country, source, license, image, and local file metadata |
| `npm run build` | Validate data and build the static site/PWA |
...
| Command | Action |
| :-- | :-- |
| `node scripts/generate-full-data.mjs` | Regenerate all 177 country drink records from `drink-seeds.tsv` and `map-countries.json` |
| `node scripts/fetch-openverse-images.mjs` | Try to replace generated fallback artwork with licensed Openverse photo downloads |
